...
Notes:
- There are some unused strings in certain AINB files that appear unreferenced by the file - those are removed when converted to JSON/YAML and back leading to certain string offsets differing between the original and converted files (if a necessary purpose is found for these strings, I add them if needed)
